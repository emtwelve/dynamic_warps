...
__device__ int test_cond ( bool x , unsigned y , void* j ) {
	...
	/* GENERATED */ printf("CALL_LOG %d test_cond bool %u unsigned %u void* %p \n", blockIdx.x * blockDim.x + threadIdx.x, x, y, j);
  if (j == NULL)
      return y + 99;
  else
      if (x)
...
__device__ int test_dev ( int x , int y , float j ) {
	...
	/* GENERATED */ printf("CALL_LOG %d test_dev int %d int %d float %f \n", blockIdx.x * blockDim.x + threadIdx.x, x, y, j);
  return x + y;
}
